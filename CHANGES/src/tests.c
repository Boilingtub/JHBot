...
char certificate_path[] = "/home/hendrik/ca-certificates/BCSCC.pem";
//char key_path[] = "/home/hendrik/ca-certificates/BCSCC.pem";
...
int main(int argc ,char* argv[]) {
    ...
    if(argc == 2) {
        if(strcmp(argv[1],"send") == 0) {
            ...
            //printf("data == %s\n",data);
            post_data("https://127.0.0.1:5444",Headers,2,data,certificate_path);
            free(data);
        } 
        else if(strcmp(argv[1],"server")==0) {
            printf("cert path : %s\n",certificate_path);
            struct SSL_Server server = SSL_Server_constructor(AF_INET,SOCK_STREAM,
                                                      0,INADDR_ANY,5444,10,certificate_path);
            while(1) {
            ...
            int listner_server = python_create_new_ssl_listner_server(
                AF_INET,SOCK_STREAM,0,INADDR_ANY,80,10,certificate_path);
            int i = 0;
            while(i < 1) {
            ...
            char* data = read_text_file("../samples/json.txt");
            for(int i = 0;i < 1; i++) {
                post_data("https://127.0.0.1",Headers,2,data,certificate_path);
            }
            free(data);
